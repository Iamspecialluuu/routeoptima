...
PAGE_META={'dashboard':('Dashboard','Overview of your delivery system'),'riders':('Riders','Manage dispatch riders and their status'),'deliveries':('Deliveries','Track and assign customer deliveries'),'optimize':('Optimize Route','Select stops and compute the shortest route'),'history':('Route History','Past route optimizations and results'),'backend':('System Architecture','How RouteOptima is built, layer by layer')}
@app.context_processor
def globals_():
    title,sub=PAGE_META.get(request.endpoint,(None,None))
    return {'google_maps_api_key':GOOGLE_MAPS_API_KEY,'page_title':title,'page_sub':sub}
@app.route('/')
def index():
    if 'admin_id' in session: return redirect(url_for('dashboard'))
    c=db()
    stats={'deliveries':c.execute('SELECT COUNT(*) FROM deliveries').fetchone()[0],'riders':c.execute("SELECT COUNT(*) FROM riders WHERE status='Available'").fetchone()[0],'total_distance':round(c.execute('SELECT COALESCE(SUM(total_distance),0) FROM route_history').fetchone()[0],1)}
...
def dashboard():
    c=db()
    stats={'riders':c.execute('SELECT COUNT(*) FROM riders').fetchone()[0],'active_riders':c.execute("SELECT COUNT(*) FROM riders WHERE status='Available'").fetchone()[0],'deliveries':c.execute('SELECT COUNT(*) FROM deliveries').fetchone()[0],'pending':c.execute("SELECT COUNT(*) FROM deliveries WHERE status='Pending'").fetchone()[0],'routes':c.execute('SELECT COUNT(*) FROM route_history').fetchone()[0],'total_distance':round(c.execute('SELECT COALESCE(SUM(total_distance),0) FROM route_history').fetchone()[0],1)}
    top_riders=c.execute('SELECT r.name,COUNT(d.id) c FROM riders r LEFT JOIN deliveries d ON d.rider_id=r.id GROUP BY r.id ORDER BY c DESC,r.name LIMIT 5').fetchall()
    days=[]
    ...
    for d in days: d['pct']=max(6,round(d['count']/max_day*100))
